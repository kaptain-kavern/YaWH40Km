...
{
    public class Verb_ShootShipWeapon : Verb_LaunchProjectile
    {
        public IntVec3 castPos;

        protected override bool TryCastShot()
        {
            if (this.castPos == null)
            {
                Log.Error("Tried Firing Ship Weapon from a null position");
                return false;
            }
            ...
            ShootLine shootLine;
            bool flag = base.TryFindShootLineFromTo(this.castPos, this.currentTarget, out shootLine);
            if (this.verbProps.stopBurstWithoutLos && !flag)
            {
                return false;
            }
            Vector3 drawPos = this.castPos.ToVector3();
            ...
            {
                float lengthHorizontalSquared = (this.currentTarget.Cell - this.castPos).LengthHorizontalSquared;
                float num;
